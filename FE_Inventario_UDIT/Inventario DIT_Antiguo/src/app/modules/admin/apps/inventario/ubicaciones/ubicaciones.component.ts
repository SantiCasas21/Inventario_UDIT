...
export class UbicacionesComponent implements OnInit, AfterViewInit {
  ...
  eliminarRegistro(row: any) {
    const nombre = row.nombre || row.ubicacion || `ID ${row.id}`;
    this.confirmacionService.confirmarEliminacion('Ubicación', nombre).subscribe(confirmado => {
      if (confirmado) {
        this.service.delete(this.endpoint, row.id).subscribe({
          ...
          error: (err: any) => {
            const msg = err.message || err.error?.message || err.error?.Message || (typeof err.error === 'string' ? err.error : 'No se puede eliminar esta ubicación porque está siendo utilizada en insumos o movimientos del sistema.');
            this.confirmacionService.mostrarAdvertencia('No es posible eliminar la ubicación', msg, `Ubicación: "${nombre}"`);
          }
        });
      }
    });
